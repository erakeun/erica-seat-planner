...
최종 Geometry에서 상단 장변·왼쪽 HEAD 연결부·하단 장변은 하나의 연속 SVG path로 구성한다. 장변의 800mm 깊이와 완만한 곡률을 유지하면서 오른쪽 두 끝과 왼쪽 내·외측 연결 모서리는 직선·각진 마감으로 표현한다. `HEAD-01` 중심 Y와 스크린 중심 Y는 동일한 `515` 좌표로 정렬한다.
