...
최종 Geometry에서 상단·하단 장변은 각자 독립된 곡선형 SVG 상판으로 구성하고, 왼쪽 HEAD 상판은 `HEAD-01`만 사용하는 별도 대형 테이블로 표시한다. 장변의 800mm 깊이와 완만한 곡률을 유지하면서 오른쪽 두 끝은 직선·각진 마감으로 표현한다. `HEAD-01` 중심 Y와 스크린 중심 Y는 동일한 `515` 좌표로 정렬한다.

## 메인 테이블 매핑

상단 `MAIN-L-01`~`MAIN-L-24`와 하단 `MAIN-R-01`~`MAIN-R-24`는 연속된 좌석 ID 순서대로 2석씩 묶는다. 따라서 상·하단은 각각 12개 가구 모듈이며, 모듈 사이 11개 경계선을 곡선 상판 안에 표시한다. `HEAD-TABLE-01`은 이 규칙에서 제외되는 독립 가구이며 좌석 ID `HEAD-01`만 대응한다. 이 매핑은 렌더링 메타데이터이고 저장 JSON의 좌석·참석자 구조에는 포함되지 않는다.
